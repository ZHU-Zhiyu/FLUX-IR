...
from data.LoL_dataset import LOLv1_Dataset, LOLv2_Dataset, Raindrop_Dataset, UnderwaterB_Dataset
...
def main():
    ...
    parser.add_argument('-use_gt_mean', '--use_gt_mean', type=bool, default=True)
    ...
    elif opt_dataset['dataset'] == 'Raindrop':
        dataset_cls = Raindrop_Dataset
    elif opt_dataset['dataset'] == 'UnderwaterB':
        dataset_cls = UnderwaterB_Dataset
    else:
    ...
    for val_data in val_loader:
        ...
        if args.stage == 'rein':
            ...
            # diffusion.test_distill(NFE=2, dataset=opt_dataset['dataset'], continous=False)
            diffusion.test_distill(NFE=1, dataset=opt_dataset['dataset'], continous=False)
        ...
        if opt_dataset['dataset'] == 'LOLv1' or opt_dataset['dataset'] == 'LOLv2':
            ...
            ll_img = ll_img[8:408, 4:604,:]
        elif opt_dataset['dataset'] == 'UnderwaterB':
            normal_img = normal_img[:gt_img.shape[0],:gt_img.shape[1],:]
        ...
        if img_mode == 'single':
            util.save_img(
                gt_img, '{}/{}_gt.png'.format(result_path_gt, val_data['GT_path'][0]))
            util.save_img(
                ll_img, '{}/{}_lq.png'.format(result_path_input, val_data['GT_path'][0]))
            util.save_img(
                normal_img, '{}/{}_enhanced.png'.format(result_path_out, val_data['GT_path'][0]))
        else:
            util.save_img(
        ...
        if args.use_gt_mean and (opt_dataset['dataset'] == 'LOLv1' or opt_dataset['dataset'] == 'LOLv2'):
            gt_img = gt_img / 255.
        ...
        if opt_dataset['dataset'] == 'LOLv1' or opt_dataset['dataset'] == 'LOLv2':
            psnr = util.calculate_psnr(normal_img, gt_img, test_y_channel=False)
            ssim = util.calculate_ssim(normal_img, gt_img, test_y_channel=False)
        elif opt_dataset['dataset'] == 'UnderwaterB':
            psnr = util.calcu_psnr(normal_img, gt_img, gray_scale=True)
            ssim = util.calcu_ssim(normal_img, gt_img, gray_scale=True)
        else:
            psnr = util.calculate_psnr(normal_img, gt_img, test_y_channel=True)
            ssim = util.calculate_ssim(normal_img, gt_img, test_y_channel=True)            
        
        # util.save_img(normal_img, '{}/{}_normal.png'.format(result_path_out, val_data['GT_path'][0]))
